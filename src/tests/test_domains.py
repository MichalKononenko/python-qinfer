...
from qinfer import ProductDomain, RealDomain, IntegerDomain, MultinomialDomain

# CONSTANTS ###################################################################

WEIRDO = np.array(
    [(1, 2., 'jump')], dtype=[('foo', 'i4'), ('bar', 'f4'), ('baz', 'S10')]
)
...
class TestPositiveRealDomain(ConcreteDomainTest, DerandomizedTestCase):
    ...
    def instantiate_good_values(self):
        return [
            0, np.pi, np.array([2.1]).astype(np.int), np.array([32.2, 2, 2.8])
        ]
...
class TestBoundedRealDomain(ConcreteDomainTest, DerandomizedTestCase):
    ...
    def instantiate_good_values(self):
        return [
            np.pi, np.array([3]), np.array([np.e, 3.13, 2.8])
        ]
...
class TestIntegerDomain(ConcreteDomainTest, DerandomizedTestCase):
    ...
    def instantiate_good_values(self):
        return [
            np.array([54]).astype(np.float),
            np.array([-32, 2, 2]).astype(np.int)
        ]
...
class TestPositiveIntegerDomain(ConcreteDomainTest, DerandomizedTestCase):
    ...
    def instantiate_good_values(self):
        return [
            3,
            np.array([54]).astype(np.float),
            np.array([0, 32, 1, 2]).astype(np.int)
        ]
...
class TestFiniteIntegerDomain(ConcreteDomainTest, DerandomizedTestCase):
    ...
    def instantiate_good_values(self):
        return [
            3,
            np.array([8]).astype(np.float),
            np.array([0, -2, 1, 2]).astype(np.int)
        ]
...
class TestMultinomialDomain(ConcreteDomainTest, DerandomizedTestCase):
    ...
    def test_array_conversion(self):
        ...
        assert_equal(
            self.domain.to_regular_array(self.domain.from_regular_array(arr2)),
            arr2
        )
        assert_equal(
            self.domain.from_regular_array(self.domain.to_regular_array(arr1)),
            arr1
        )
        
        
class TestIntegerIntegerProductDomain(
    ConcreteDomainTest, DerandomizedTestCase
):
    """
    ...
    def test_array_conversion(self):
        ...
        assert_equal(
            self.domain.to_regular_arrays(
                self.domain.from_regular_arrays(arr2)), arr2
        )
        assert_equal(
            self.domain.from_regular_arrays(
                self.domain.to_regular_arrays(arr1)), arr1
        )
...
class TestIntegerIntegerProductDomain2(
    ConcreteDomainTest, DerandomizedTestCase
):
    """
    Tests ProductDomain([IntegerDomain, IntegerDomain])
...
class TestIntegerMultinomialProductDomain(
    ConcreteDomainTest, DerandomizedTestCase
):
    """
    Tests ProductDomain([IntegerDomain, IntegerDomain])
    ...
    def test_array_conversion(self):
        ...
        arr2 = [
            np.array([5, 0]),
            np.array([([1, 2, 2],), ([5, 0, 0],)], dtype=[('k', '<i8', 3)])
        ]
        ...
        assert_equal(
            self.domain.to_regular_arrays(
                self.domain.from_regular_arrays(arr2)
            ),
            arr2
        )
        assert_equal(
            self.domain.from_regular_arrays(
                self.domain.to_regular_arrays(arr1)
            ),
            arr1
        )
